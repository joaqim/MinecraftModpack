...
def write_pack(path: Path, pack: PackInfo) -> None:
    ...
    # Convert the PackInfo dataclass instance to a dictionary
    pack_dict = dacite.to_dict(pack)
    # Write the dictionary to the file in TOML format
    with open(path, "wb") as pack_file:
        tomli.dump(pack_dict, pack_file)
...
def increment_tag(current_tag, commit_msg):
    ...
    if match:
        ...
        print([type_value, scope_value, description_value])

        if "release" in [type_value, scope_value] or "major" in scope_value:
